...
function haeTiedot(){
	...
	fetch("veneet/" + document.getElementById("hakusana").value,{//Lähetetään kutsu backendiin
	      method: 'GET'
...
function poista(tunnus, nimi, merkkimalli){
	if(confirm("Poista vene " + tunnus + " "+ nimi +" " + merkkimalli+" ?")){	
		fetch("veneet/"+tunnus,{//Lähetetään kutsu backendiin
		      method: 'DELETE'		      	      
		    })
		.then(function (response) {//Odotetaan vastausta ja muutetaan JSON-vastaus objektiksi
			return response.json()
		...
		.then(function (responseJson) {//Otetaan vastaan objekti responseJson-parametrissä		
			...
			if(vastaus==0){
				document.getElementById("ilmo").innerHTML= "Venen poisto epäonnistui.";
	        }else if(vastaus==1){	        	
