...
document.getElementById("nimi").focus();//viedään kursori Nimi-kenttään sivun latauksen yhteydessä

//Haetaan muutettavan veneen tiedot. Kutsutaan backin GET-metodia ja välitetään kutsun mukana muutettavan veneen tunnus
//GET /veneet/haeyksi/tunnus
var tunnus = requestURLParam("tunnus"); //Funktio löytyy scripts/main.js 
fetch("veneet/haeyksi/" + tunnus,{//Lähetetään kutsu backendiin
      method: 'GET'	      
    })
...
.then( function (responseJson) {//Otetaan vastaan objekti responseJson-parametrissä	
	...
	document.getElementById("nimi").value = responseJson.nimi.replace(/&nbsp;/g, ' ');	//muokataan syötekenttään välilyönti takaisin
	document.getElementById("merkkimalli").value = responseJson.merkkimalli.replace(/&nbsp;/g, ' '); //samoin tähän kenttään
...
function vieTiedot(){	
	...
	var pituus = siivoa(document.getElementById("pituus").value); //korvataan syötteen merkki ennen validointia
	var leveys = siivoa(document.getElementById("leveys").value); 
	var hinta = document.getElementById("hinta").value;
	
	if(nimi.length<2||merkkimalli.length<2||pituus*1!=pituus||leveys*1!=leveys||hinta*1!=hinta){ //validoidaan arvot, luvut kerrotaan itsellään lu
	...
	fetch("veneet",{//Lähetetään kutsu backendiin
	      method: 'PUT',
	      body:formJsonStr
	    })
